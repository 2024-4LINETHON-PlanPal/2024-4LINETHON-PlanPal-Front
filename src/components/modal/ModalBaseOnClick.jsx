...
export default function ModalBaseOnClick({
  // 모달 닫는 함수
  setCloseModal,
  // 모달 내부 내용(컴포넌트)
  InsideComponent,
    ...
    <>
      ...
      <A.ModalContainer style={{ top: `calc(50% - (${modalHeight} / 2))` }}>
        <A.ModalTopbarContainer>
          <A.ImgContainer onClick={setCloseModal}>
        ...
        <A.ModalVerticalButtonContainer>
          ...
          {longSingleBtnText1 && (
            <A.LongSingleButton1 onClick={onLongSingleBtnClick}>{longSingleBtnText1}</A.LongSingleButton1>
          )}
          {longSingleBtnText2 && (
            <A.LongSingleButton2 onClick={onLongSingleBtnClick2}>{longSingleBtnText2}</A.LongSingleButton2>
          )}
          {/* 세로가 긴 버튼 - 친구 추가하기 */}
